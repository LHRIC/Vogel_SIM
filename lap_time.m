...
[LT,RT,KT] = curvature(track_points'); % outputs radius of curvature (RT) LT and KT not used
RT = RT(~isnan(RT)); % deleteing any points that have zero radius or undefined curvature
for i = 1:length(RT)
    segment(i) = i; % segmenting the track based on number of radi points
    r = max(r_min,RT(i)); % define min and max radius of car
    r = min(r,r_max);
    RT(i) = r; % actual radius of the car for every track radius
    Vmax(i) = min(VMAX,fnval(cornering,r)); % determine if the car is grip or power limited for every track point (maximum speed)
    x1(i) = track_points(1,i+1); 
    x2(i) = track_points(1,i+2);
    y1(i) = track_points(2,i+1);
    y2(i) = track_points(2,i+2);
    dist(i) = sqrt((x1(i)-x2(i))^2+(y2(i)-y1(i))^2); % find distance between next two points
end
%%
...
count = 0; % init count
    v = 20; % first velocity to shift at
    vel = v;
    gears = find((shift_points-vel)>0); % shift_points is the velocity of every shift point
    gear = gears(1)-1; % intilizing the first gear used as the gear before 20 ft/sec
    newgear = gear; % the first gear used
    time_shifting = 0; % no shift at the start
for i = 1:1:length(segment) % for every curvature defined in the track
    d = dist(i);

    r = RT(i); % Car radius at every track point
    %gear = newgear;
    gears = find((shift_points-vel)>0); % determine if you can shift
    newgear = gears(1)-1; 
    
    if newgear > gear % update shift count
        shifting = 1;
    else
        shifting = 0;
    end

    vmax = min(VMAX,fnval(cornering,r)); % determine if the car is grip or power limited for every track point (maximum speed)
    if vmax < 0
        vmax = VMAX;
    end
    AX = fnval(accel,vel);
